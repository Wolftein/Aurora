...
namespace UI
{
    ...
    {
        return SciterProcX(mHandle, SCITER_X_MSG_KEY(KEY_CHAR, Codepoint, KEYBOARD_STATES(mKeys)));
    }
    ...
    Bool SciterInput::OnKeyDown(Input::Key Key)
    {
        ...
        {
            mKeys |= SHIFT_KEY_PRESSED;
        }
        else if (Key == Input::Key::RightShift)
        {
            mKeys |= RIGHT_SHIFT_KEY_PRESSED;
        }
        else if (Key == Input::Key::LeftCtrl || Key == Input::Key::RightCtrl)
        {
            mKeys |= CONTROL_KEY_PRESSED;
        }
        else if (Key == Input::Key::LeftAlt || Key == Input::Key::RightAlt)
        {
            mKeys |= ALT_KEY_PRESSED;
        }

        if (SciterProcX(mHandle, SCITER_X_MSG_KEY(KEY_DOWN, GetKey(Key), KEYBOARD_STATES(mKeys))))
        {
    ...
    Bool SciterInput::OnKeyUp(Input::Key Key)
    {
        ...
        if (SciterProcX(mHandle, SCITER_X_MSG_KEY(KEY_UP, GetKey(Key), KEYBOARD_STATES(mKeys))))
        {
            sciter::dom::element Element(sciter::dom::element::focus_element(NULL));
            return Element.root() != Element.operator HELEMENT();
        }
    ...
    Bool SciterInput::OnMouseMove(UInt X, UInt Y)
    {
        ...
        mMousePosition.y = Y;
        return SciterProcX(
            mHandle, SCITER_X_MSG_MOUSE(MOUSE_MOVE, mMouseButtons, KEYBOARD_STATES(mKeys), mMousePosition));
    }
    ...
    Bool SciterInput::OnMouseScroll(SInt X, SInt Y)
    {
        ...
        return SciterProcX(
            mHandle, SCITER_X_MSG_MOUSE(MOUSE_WHEEL, MOUSE_BUTTONS(Deltas), KEYBOARD_STATES(mKeys), mMousePosition));
    }
    ...
    Bool SciterInput::OnMouseUp(Input::Button Button)
    {
        ...
        return SciterProcX(
            mHandle, SCITER_X_MSG_MOUSE(MOUSE_UP, GetButton(Button), KEYBOARD_STATES(mKeys), mMousePosition));
    }
    ...
    Bool SciterInput::OnMouseDown(Input::Button Button)
    {
        ...
        return SciterProcX(
            mHandle, SCITER_X_MSG_MOUSE(MOUSE_DOWN, mMouseButtons, KEYBOARD_STATES(mKeys), mMousePosition));
    }
    ...
    {
        return SciterProcX(
            mHandle, SCITER_X_MSG_MOUSE(MOUSE_DCLICK, mMouseButtons, KEYBOARD_STATES(mKeys), mMousePosition));
    }
